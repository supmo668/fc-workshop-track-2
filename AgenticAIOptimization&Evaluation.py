...
from workshop.command import Command, CommandExecutor, ServiceType
...
class AgentDecisionAssessment(BaseModel):
    ...
    decision_score: float = Field(
        description="Decision quality score (0.0-1.0)", ge=0.0, le=1.0
    )
    reasoning_quality: float = Field(
        description="Quality of reasoning behind decisions (0.0-1.0)", ge=0.0, le=1.0
    )
    action_appropriateness: float = Field(
        description="How appropriate actions are for scenario (0.0-1.0)", ge=0.0, le=1.0
    )
    resource_efficiency: float = Field(
        description="Efficiency of resource utilization (0.0-1.0)", ge=0.0, le=1.0
    )
    coordination_effectiveness: float = Field(
        description="How well agent coordinates with others (0.0-1.0)", ge=0.0, le=1.0
    )
    improvement_suggestions: List[str] = Field(
        description="Specific suggestions for improvement"
    )
    strengths: List[str] = Field(description="Key strengths demonstrated by the agent")
...
class AdvancedEvaluationFramework:
    ...
    def llm_as_judge_evaluation(
        self, agent_result, scenario: ScenarioDefinition, commands: List[Command]
...
Provide specific, actionable feedback for improvement."""
    ...
    def evaluate_decision_quality(
        self, commands: List[Command], scenario: ScenarioDefinition
    ) -> float:
        """
        ...
        enhanced_score = (
            (base_score * 0.5) + (quality_factors * 0.3) + (llm_score * 0.2)
        )
    ...
    def _analyze_decision_patterns(
        ...
        coverage_score = len(services_used.intersection(expected_services)) / len(
            expected_services
        )
        ...
        emergency_commands = [
            cmd for cmd in commands if cmd.service == ServiceType.EMERGENCY
        ]
        ...
        quality_score = (
            coverage_score * 0.4 + sequence_score * 0.3 + efficiency_score * 0.3
        )
    ...
    def _evaluate_resource_efficiency(self, commands: List[Command]) -> float:
        ...
        drone_assignments = len(
            [cmd for cmd in commands if cmd.action == "assign_drone"]
        )
    ...
    def measure_response_time(
        self, agent_function, *args, **kwargs
    ) -> Tuple[Any, float]:
        """
    ...
    def _analyze_resource_coordination(self, commands: List[Command]) -> float:
        ...
        self.performance_trends["resource_coordination"] = {
            ...
            "coordination_quality": (
                ...
                else "moderate" if coordination_score > 0.4 else "poor"
            ),
        }
    ...
    def create_comprehensive_evaluation(
        self, agent_result, scenario: ScenarioDefinition, response_time: float
    ) -> EvaluationMetrics:
        """
                ...
                execution_details.append(
                    {
                        ...
                        "success": success,
                        "parameters_count": (
                            len(cmd.parameters) if cmd.parameters else 0
                        ),
                    }
                )
            except Exception as e:
...
console.print(Panel("📏 Phase 1: Evaluation Framework Setup", border_style="blue"))
...
console.print(
    Panel("⚡ Phase 2: Performance Baseline Measurement", border_style="blue")
)
...
baseline_logger = EvaluationLogger(
    model="baseline_agent_system", dataset="optimization_workshop_scenarios"
)
...
for scenario_name in EVALUATION_CONFIG["scenarios_to_test"]:
    if scenario_name in evaluation_scenarios:
        ...
        pred_logger.log_score(
            scorer="execution_efficiency", score=metrics.execution_efficiency
        )
        pred_logger.log_score(
            scorer="adaptability_score", score=metrics.adaptability_score
        )
        pred_logger.log_score(
            scorer="coordination_quality", score=metrics.coordination_quality
        )
        pred_logger.log_score(
            scorer="user_satisfaction", score=metrics.user_satisfaction
        )
...
console.print(
    Panel("🔌 Phase 3: Model Context Protocol (MCP) Integration", border_style="purple")
)
...
class MCPServiceRegistry:
    ...
    def simulate_runtime_discovery(self):
        ...
        for service in self.config["discovery_simulation"]["external_services"]:
            ...
            if random.random() < success_rate:
                ...
                log_entry = (
                    f"❌ MCP Integration Failed: {service['name']} (connection timeout)"
                )
                self.integration_log.append(log_entry)
                console.print(log_entry)

        console.print(
            f"🎯 MCP Discovery Complete: {discovered_count}/{len(self.config['discovery_simulation']['external_services'])} services integrated"
        )
    ...
    def create_mcp_enhanced_agent(
        self, base_agent_creator, agent_type="crisis_coordinator"
    ):
        """Create agent with MCP-discovered capabilities."""

        # Calculate total available capabilities through MCP
        static_capabilities = sum(
            len(tool["capabilities"]) for tool in self.static_tools.values()
        )
...
• Always prefer higher-reliability MCP services when available"""
    ...
    def demonstrate_adaptive_behavior(self, scenario):
        ...
        console.print(
            f"• Total Capabilities: {sum(len(tool['capabilities']) for tool in self.static_tools.values())}"
        )
        ...
        if self.capability_matrix:
            ...
            for capability, services in list(self.capability_matrix.items())[:5]:
                console.print(
                    f"• {capability.replace('_', ' ').title()}: {len(services)} service(s) available"
                )

        # Create MCP-enhanced agent
        enhanced_agent = self.create_mcp_enhanced_agent(
            create_optimized_grid_agent, agent_type="mcp_crisis_coordinator"
        )
    ...
    def get_mcp_integration_report(self):
        ...
        return {
            ...
            "total_capabilities": (
                sum(len(tool["capabilities"]) for tool in self.static_tools.values())
                + sum(
                    len(service["capabilities"])
                    for service in self.discovered_services.values()
                )
            ...
            "integration_success_rate": (
                ...
                if self.config["discovery_simulation"]["external_services"]
                else 0
            ),
...
mcp_enhanced_logger = EvaluationLogger(
    model="mcp_enhanced_agents", dataset="dynamic_service_discovery"
)
...
mcp_report = mcp_registry.get_mcp_integration_report()
mcp_pred_logger.log_score(
    scorer="mcp_integration_success_rate", score=mcp_report["integration_success_rate"]
)
mcp_pred_logger.log_score(
    scorer="total_capabilities", score=mcp_report["total_capabilities"]
)
mcp_pred_logger.log_score(
    scorer="discovered_services_count", score=mcp_report["discovered_services"]
)
...
console.print(
    "✅ Phase 3 Complete: MCP integration enables dynamic agent extensibility"
)
...
console.print(Panel("🔄 Phase 4: Human Feedback Integration", border_style="blue"))
...
class InteractiveHumanFeedbackSystem:
    ...
    def simulate_expert_feedback(self, agent_actions, scenario):
        ...
        safety_actions = len(
            [
                ...
                and cmd.action in ["set_priority", "assign_drone"]
            ]
        )
        base_safety = min(0.9, 0.4 + (safety_actions * 0.15))
        expert_bias = self.config["feedback_simulation"]["expert_bias"]
        feedback_scores["safety_priority"] = base_safety * expert_bias + base_safety * (
            1 - expert_bias
        )
        ...
        service_types = len(
            set(
                getattr(cmd, "service", "unknown").value
                for cmd in agent_actions
        ...
        overall_score = sum(
            score * criteria[criterion] for criterion, score in feedback_scores.items()
        )
    ...
    def _generate_detailed_feedback(self, scores, overall_score):
        ...
        if overall_score >= thresholds["excellent_score"]:
            feedback.append(
                "🌟 Excellent performance! This response demonstrates mastery."
            )
            ...
            feedback.append(
                "❌ Performance below standards. Major improvements needed."
            )
    ...
    def _apply_feedback_enhancements(self, agent, patterns, agent_type):
        ...
        if goal_enhancements:
            enhanced_goal = (
                f"{original_goal}, while ensuring you {', '.join(goal_enhancements)}"
            )
...
5. Coordinate with grid and emergency teams"""
    ...
    def analyze_feedback_patterns(self):
        ...
        for criterion in criteria:
            scores = [
                session["feedback"]["scores"][criterion]
        ...
        return {
            ...
            "overall_trend": (
                ...
                and self.feedback_history[-1]["feedback"]["overall_score"]
                > self.feedback_history[0]["feedback"]["overall_score"]
                else "stable"
            ),
        }
...
feedback_enhanced_crew = Crew(
    ...
    tasks=create_optimized_agent_tasks(
        enhanced_grid_agent,
        enhanced_emergency_agent,
        enhanced_traffic_agent,
        test_scenario,
    ),
    process=Process.sequential,
    verbose=False,
...
feedback_enhanced_logger = EvaluationLogger(
    model="feedback_enhanced_agents", dataset="human_feedback_scenarios"
)

agent_result = feedback_enhanced_crew.kickoff(
    inputs={
        "scenario_name": test_scenario.name,
        "scenario_description": test_scenario.description,
    }
...
# Log feedback scores
feedback_pred_logger.log_score(
    scorer="feedback_overall_score", score=feedback_result["overall_score"]
)
...
console.print(
    Panel(
        ...
        + "\n".join(
            [
                f"• {criterion.replace('_', ' ').title()}: {score:.3f}"
                for criterion, score in feedback_result["scores"].items()
            ]
        )
        + "\n\n"
...
class ComprehensiveWorkshopEvaluator:
    ...
    def run_comprehensive_evaluation(
        self, participant_name, optimization_config, participant_agents=None
    ):
        """
        ...
        console.print(
            "📊 This evaluation measures 5 key dimensions of agent performance..."
        )
        ...
        optimization_results = self._evaluate_optimization_techniques(
            optimization_config
        )
    ...
    def _evaluate_optimization_techniques(self, optimization_config):
        ...
        if "latency_optimization" in optimization_config:
            latency_improvement = optimization_config["latency_optimization"].get(
                "improvement", 0
            )
    ...
    def _calculate_innovation_score(self, optimization_config):
        ...
        return {"score": base_score, "factors": innovation_factors}
    ...
    def _calculate_final_score(
        self, baseline, optimization, feedback, innovation, consistency
    ):
        """
        ...
        innovation_score = (
            innovation["score"] if isinstance(innovation, dict) else innovation
        )
    ...
    def _update_evaluation_stats(self, evaluation_entry):
        ...
        for metric, value in breakdown.items():
            ...
            if isinstance(value, dict) and "overall_score" in value:
                self.evaluation_stats["average_scores"][metric].append(
                    value["overall_score"]
                )
    ...
    def display_leaderboard(self):
        ...
        for i, entry in enumerate(self.leaderboard[:10], 1):
            ...
            top_area = max(
                ...
                key=lambda x: (
                    x[1]["score"]
                    if isinstance(x[1], dict) and "score" in x[1]
                    else x[1] if isinstance(x[1], (int, float)) else 0
                ),
            )
    ...
    def _get_average_final_score(self):
        ...
        if self.leaderboard:
            return sum(entry["final_score"] for entry in self.leaderboard) / len(
                self.leaderboard
            )
        return 0.0
    ...
    def _get_winner_strength_analysis(self, winner):
        ...
        for area, value in breakdown.items():
            score = (
                ...
                else value if isinstance(value, (int, float)) else 0
            )
            if score > max_score:
                max_score = score
                strongest_area = area
...
for demo in demo_configurations:
    ...
    competition_logger = EvaluationLogger(
        model=model_name, dataset="workshop_optimization_competition"
    )
    evaluation_result = workshop_evaluator.run_comprehensive_evaluation(
        demo["participant"], demo["config"]
    )
    ...
    competition_pred_logger.log_score(
        scorer="final_competition_score", score=evaluation_result["final_score"]
    )
    ...
    for category, score_data in breakdown.items():
        if isinstance(score_data, dict):
            if "overall_score" in score_data:
                competition_pred_logger.log_score(
                    scorer=f"competition_{category}", score=score_data["overall_score"]
                )
            elif "score" in score_data:
                competition_pred_logger.log_score(
                    scorer=f"competition_{category}", score=score_data["score"]
                )
        elif isinstance(score_data, (int, float)):
            competition_pred_logger.log_score(
                scorer=f"competition_{category}", score=score_data
            )
...
console.print(
    Panel(detailed_analysis, title="🏆 WINNER ANALYSIS", border_style="yellow")
)
...
final_workshop_results = {
    ...
    "human_feedback": (
        feedback_system.analyze_feedback_patterns()
...
console.print(
    "🎓 Thank you for participating in the comprehensive optimization challenge!"
)
